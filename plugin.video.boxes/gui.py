...
if settings.getSetting('theme') == '0':
    ...
    fanart = xbmc.translatePath(os.path.join('http://www.boxes.com.mt/addons/images/fanart/fanart.jpgsports.jpg', ''))
else:
...
def CheckVersion():
   try:        
    curver=xbmc.translatePath(os.path.join('special://home/addons/plugin.video.boxes/','addon.xml'))    
    source= open( curver, mode = 'r' )
    link = source . read( )
    source . close ( )
    match=re.compile('" version="(.+?)" name="Boxes"').findall(link)
    for vernum in match:
