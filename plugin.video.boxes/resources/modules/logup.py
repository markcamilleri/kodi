...
ADDON_TITLE = settings.getAddonInfo('name')
...
class LogUploader(object):
    ...
    def get_settings(self):
        self.email_address = settings.getSetting('email')
    ...
    def ask_upload(self, logfile):
        ...
        if self.email_address:
            ...
            msg2 = ('                 Take note of following ID or url to post in forum')
        return Dialog.yesno(ADDON_TITLE + ' Log Uploader', msg1, '', msg2)

    def report_msg(self, paste_id):
        url = UPLOAD_LINK % paste_id
        Dialog = xbmcgui.Dialog()
        msg1 = ('ID: [COLOR gold][B]%s[/B][/COLOR]') % paste_id
        msg2 = ('URL: %s') % url
        return Dialog.ok(ADDON_TITLE +' Log Uploader', msg1, '', msg2)
