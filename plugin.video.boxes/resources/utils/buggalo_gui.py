...
mdbAddon = xbmcaddon.Addon(id='plugin.video.boxes')
...
class BuggaloDialog(xbmcgui.WindowXMLDialog):
    ...
    def __new__(cls, serviceUrl, gmailRecipient, heading, data):
        return super(BuggaloDialog, cls).__new__(cls, 'buggalo-dialog.xml', mdbAddon.getAddonInfo('path'))
    ...
    def onInit(self):
        ...
        self.getControl(self.DETAILS_LINE1_LABEL).setLabel(mdbAddon.getLocalizedString(91000) % self.data['addon']['name'].decode('utf-8', 'replace'))
        ...
        try:
            ...
            item = xbmcgui.ListItem(label2=mdbAddon.getLocalizedString(91008))
            listControl.addItem(item)
    ...
    def onClick(self, controlId):
        if controlId == self.CLOSE_BUTTON:
            ...
            try:
                ...
                self.getControl(self.THANK_YOU_LABEL).setLabel(mdbAddon.getLocalizedString(91009))
            ...
            if self.detailsVisible:
                self.getControl(self.DETAILS_BUTTON).setLabel(mdbAddon.getLocalizedString(91007))
            else:
                self.getControl(self.DETAILS_BUTTON).setLabel(mdbAddon.getLocalizedString(91006))
